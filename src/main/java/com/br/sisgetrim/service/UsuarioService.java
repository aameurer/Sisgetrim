...
public class UsuarioService implements UserDetailsService {
     ...
     * 2. Fallback para a primeira entidade vinculada ao usuário.
     */
    public Entidade getEntidadeSelecionada(Usuario usuarioLogado, jakarta.servlet.http.HttpSession session) {
        if (usuarioLogado == null)
            return null;

        // Recarregar usuário para garantir acesso às coleções Lazy
        Usuario usuario = buscarPorDocumento(usuarioLogado.getDocumento());
        Long entidadeSelecionadaId = (Long) session.getAttribute("entidadeSelecionadaId");

        if (entidadeSelecionadaId != null) {
            return usuario.getEntidades().stream()
                    .filter(e -> e.getId().equals(entidadeSelecionadaId))
                    .findFirst()
                    .orElseGet(() -> usuario.getEntidades().stream().findFirst().orElse(null));
        }

        Entidade primeira = usuario.getEntidades().stream().findFirst().orElse(null);
        if (primeira != null && session.getAttribute("entidadeSelecionadaId") == null) {
            session.setAttribute("entidadeSelecionadaId", primeira.getId());
        }
        return primeira;
    }
